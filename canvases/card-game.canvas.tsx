...
function Lobby({ onStart, onRules }: { onStart: (count: number) => void; onRules: () => void }) {
  ...
  return (
    <FeltShell center style={{ padding: "32px 20px" }}>
      <div
        style={{
          width: "100%",
          maxWidth: 440,
...
function Table({
  ...
  const hasAnyLegal =
    isHumanTurn &&
    ((humanZone === "hand" && (handLegal?.some(Boolean) || faceUpLegal?.some(Boolean))) ||
      (humanZone === "faceUp" && faceUpLegal?.some(Boolean)) ||
      humanZone === "faceDown");
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Spacer />
        <Text style={{ color: theme.text.secondary, fontSize: 12 }}>{statusMsg}</Text>
      </div>
      ...
      {(finishOrder.length > 0 || phase === "finished") && (
        ...
        >
          {phase === "finished" ? (
            <div style={{ fontWeight: 700, marginBottom: 6 }}>Итоги</div>
          ) : (
            <div style={{ fontWeight: 600, marginBottom: 4, color: theme.text.secondary }}>
      ...
      >
        ...
        >
          ...
          <SeatLabel
            name={players[0].name}
            isHuman
            ready={isSwap ? humanReady : undefined}
        ...
        <div style={{ marginTop: 14, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          {!humanReady && (
            <Text style={{ color: theme.text.secondary, fontSize: 12 }}>
              Кликни карту в руке, затем открытую на столе (1–3 раза)
            </Text>
          )}
          <button
            onClick={onReady}
      ...
      {isHumanTurn && (
        <div style={{ marginTop: 14, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
          <Text style={{ color: theme.text.secondary, fontSize: 12 }}>
            {pickupAwaitTable
              ? humanZone === "faceDown"
                ? "Кликни закрытую карту со стола — заберёшь её вместе со сбросом"
                : "Кликни открытую карту — заберёшь все такого ранга вместе со сбросом"
              : humanZone === "faceDown"
                ? "Кликни закрытую карту — она откроется и сыграет (или заберёшь сброс)"
                : selectionCount(playSelected) === 0
                  ? humanZone === "hand"
                    ? combineOk
                      ? "Последние карты в руке — можно добавить открытые того же ранга"
                      : "Выбери карты одного ранга из руки, затем «Ход»"
                    : "Выбери открытые карты одного ранга, затем «Ход»"
                  : `Выбрано: ${selectionCount(playSelected)} × ${selRank}${
                      playSelected.faceUp.length > 0 && playSelected.hand.length > 0
                        ? " (рука + стол)"
                        : ""
                    }`}
          </Text>
                ...
                {humanZone !== "faceDown" && (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: 8,
                      border: "none",
                      background: canPlaySelected ? theme.accent.control : theme.fill.tertiary,
                      color: canPlaySelected ? "#fff" : theme.text.tertiary,
                      fontSize: 14,
                      fontWeight: 700,

...
function Hand({
  ...
  const w = cardW ?? (small ? 38 : 56);
  const h = cardH ?? (small ? 54 : 78);
  const overlap = Math.round(w * (isOwner ? 0.3 : 0.42));
  const n = cards.length;
  const comfortableStep = w - overlap;
  const minPeek = peek ?? (isOwner ? comfortableStep : Math.max(11, Math.round(w * 0.3)));
  const naturalW = n <= 1 ? w : w + (n - 1) * comfortableStep;
  const cap = maxWidth ?? (isOwner ? Infinity : w + Math.max(0, n - 1) * Math.max(minPeek, comfortableStep));
  const packedStep = n <= 1 ? 0 : (Math.min(cap, Math.max(naturalW, w + (n - 1) * minPeek)) - w) / (n - 1);
  const step = n <= 1 ? 0 : Math.max(minPeek, n > 3 || naturalW > cap ? packedStep : comfortableStep);
...
function Table({
  ...
  const oppH = short ? Math.round(clamp(vp.h * 0.15, 40, 54)) : 58;
  ...
  const hint = isSwap
    ? humanReady
      ? ""
      : "Кликни карту в руке, затем открытую на столе"
    : isHumanTurn
      ? pickupAwaitTable
        ? humanZone === "faceDown"
          ? "Кликни закрытую карту со стола"
          : "Кликни открытую карту того же ранга"
        : humanZone === "faceDown"
          ? "Кликни закрытую карту"
          : selectionCount(playSelected) === 0
            ? humanZone === "hand"
              ? combineOk
                ? "Можно добавить открытые того же ранга"
                : "Выбери карты одного ранга"
              : "Выбери открытые одного ранга"
            : `Выбрано: ${selectionCount(playSelected)} × ${selRank}`
      : "";

  function renderOpp(pIdx: number, side: "left" | "right" | "top") {
    const hand = playerHand(pIdx);
    const peek = Math.max(side === "top" ? 10 : 12, Math.round(oppW * 0.3));
    const maxW = oppW + Math.max(0, hand.length - 1) * peek;
    return (
      <div
        key={pIdx}
        style={{
          ...
          alignItems: "center",
          gap: 3,
          padding: side === "top" ? "0 8px" : "2px 4px",
        }}
      >
        <SeatLabel
      ...
      >
        ...
        <div
          style={{
            ...
            justifyContent: "center",
            gap: 10,
            minWidth: seats.left.length ? 96 : 0,
            minHeight: 0,
          }}
        >
        ...
        <div
          style={{
            ...
            justifyContent: "center",
            gap: 10,
            minWidth: seats.right.length ? 96 : 0,
            minHeight: 0,
          }}
        >
